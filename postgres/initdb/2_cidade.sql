...
CREATE OR REPLACE FUNCTION normalize_column(table_name text, column_name text)
    RETURNS void AS $$
DECLARE
    sql_stmt text;
BEGIN
    sql_stmt := 'UPDATE ' || quote_ident(table_name) || ' SET ' || quote_ident(column_name) || ' = unaccent(''unaccent'', ' || quote_ident(column_name) || ')';
    EXECUTE sql_stmt;
END;
$$ LANGUAGE plpgsql;
...
DO $$
    BEGIN
        PERFORM normalize_column('tb_cidade', 'nome');
        PERFORM normalize_column('tb_estado', 'nome');
    END;
$$;